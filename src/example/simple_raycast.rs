...
fn main() {
  App::new()
      ...
      .add_systems(
          First,
          (update_raycast_with_cursor_position.before(RaycastSystem::BuildRays::<MyRaycastSet>),update_raycast_position ),
      )
...
fn update_raycast_position(
  ...
  for mut pick_source in &mut query {
      ...
      //if let Some(cursor_latest) = cursor.iter().last() {
          //pick_source.cast_method = RaycastMethod::Screenspace(cursor_latest.position);
      //}
      //pick_source.ray.unwrap().direction()
      //if let Some(myray ) = pick_source.ray {
        //println!("DIR: {:?}", myray.direction());//screen
        //println!("origin: {:?}", myray.origin());
        //println!("position: {:?}", myray.position(10.0));
      //}

      if let Some((_entity,interset )) = pick_source.get_nearest_intersection() {
        //println!("normal: {:?}", interset.normal());
        println!("position: {:?}", interset.position());
